...
9. **➕ 自治体追加リクエスト**: 世帯数レイヤの凡例・画面上部のリンクから、未収録自治体の追加をGitHub Issueでリクエストできる(区割の正確な記載を依頼・反映は約束しない旨を明記)
   ...
   - 追加方法: e-Stat 統計GISの境界データ(世界測地系緯度経度・shape形式)をDLし
     - 1市区町村: `python3 build/make_setai_geojson.py <r2kaXXXXX.shp> data/setai_<code>.geojson`
     - 都道府県一括(code=2桁でDL): `python3 build/make_setai_geojson.py <r2kaXX.shp> --split data/`
   ...
   - ⚠️**互換性の約束**: `data/setai_<code>.geojson` の**パスと properties 名(`name`/`setai`/`jinko`/`key`/`units`)は変えない・消さない**。配布済みHTMLがこのパスを直接叩いているため、リネームや削除は手元のファイルを壊す。項目の追加は安全(古い版は無視するだけ)
