...
9. **➕ 自治体追加リクエスト**: 世帯数レイヤの凡例・画面上部のリンクから、未収録自治体の追加をGitHub Issueでリクエストできる(区割の正確な記載を依頼・反映は約束しない旨を明記)
   ...
   - **飛び地の扱い**: 1つの町丁目が複数ポリゴンに分かれる場合、e-Stat は世帯数をそのうち1つだけに載せ残りを0にする。変換スクリプトは同一 `KEY_CODE` を1フィーチャ(MultiPolygon)へ統合してこれを吸収する(未統合だと飛び地が「0世帯」と誤表示される)。詳細は `build/make_setai_geojson.py` の docstring
