...
const SETAI_METRICS = [
  ...
  { id: "jinko", grp: "基本", label: "人口総数", unit: "人", val: p => p.jinko },
...
function addSetaiLayers(map, opts) {
  ...
  function popupHtml(p) {
    ...
    let html = `<b>${p.name}</b><br><table style="border-collapse:collapse;font-size:12px">` +
      row("世帯数", p.setai, "世帯") + row("人口", p.jinko, "人");
    if (p.hi) {
      html += `</table>` + gray("※男女・外国人・年齢別は非公表(少人口のため国勢調査が別地域へ合算=秘匿処理)") ;
    } else {
      ...
      if (p.tt != null && p.tt !== p.jinko) notes.push(`※男女・年齢等の母数は統計表の総数${p.tt.toLocaleString()}人`);
