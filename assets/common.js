...
/* ===== 表示指標の切替(2026-09 属性追加) =====
   GeoJSON properties(make_setai_geojson.py --stats が付与):
     m/f=男女, gk=外国人, a0/a1/a2/a75=15歳未満/15〜64/65歳以上/75歳以上,
     ag=[5歳階級21値(0〜4…100歳以上・総数)], nf=年齢不詳, tt=表2総数(jinkoと違うときのみ),
     hi=1 秘匿等データなし, gs=1 合算受け皿(秘匿地域の人数を含む)
   ・日本人人口は直接の統計が無いため「総数−外国人」の概算(国籍不詳を含む)
   ・「未成年(18歳未満)」は5歳階級から作れない(15〜19歳が分割不能)ので置かない */
const SETAI_RAMP = ["#eff3ff", "#c6dbef", "#9ecae1", "#6baed6", "#3182bd", "#08519c"]; /* 淡→濃 */
const SETAI_NODATA = "#d9dde3";      /* 秘匿等データなしの塗り */
const SETAI_AGES = ["0〜4歳", "5〜9歳", "10〜14歳", "15〜19歳", "20〜24歳", "25〜29歳",
  "30〜34歳", "35〜39歳", "40〜44歳", "45〜49歳", "50〜54歳", "55〜59歳", "60〜64歳",
  "65〜69歳", "70〜74歳", "75〜79歳", "80〜84歳", "85〜89歳", "90〜94歳", "95〜99歳", "100歳以上"];
function setaiTT(p){ return (p.tt != null ? p.tt : p.jinko) || 0; }
function setaiPct(a, b){ return b > 0 ? Math.round(a / b * 1000) / 10 : null; }
/* val: properties(+選択中の年齢階級index) -> 数値 or null(データなし) */
const SETAI_METRICS = [
  { id: "setai", grp: "基本", label: "世帯数", unit: "世帯", val: p => p.setai },
  /* 人口総数は統計表の総数(tt。秘匿合算の受け皿では境界のjinkoより大きい)を優先し、
     男女・年齢・外国人の各指標と母数を揃える(C-2: 基準の混在で 8人→65歳以上38人 のような
     読み違いが起きないように)。ttが無い feature では jinko と同値 */
  { id: "jinko", grp: "基本", label: "人口総数", unit: "人", val: p => p.hi ? p.jinko : setaiTT(p) },
...
/* 表示中の値集合から6段階の等分位ビンを作る(指標ごとにスケールが違うため固定閾値にしない)。
   戻り値は SETAI_BINS と同じ {min,color,label} の降順配列。世帯数だけは従来の固定ビンを使う */
function setaiMakeBins(values, unit){
  const vs = values.filter(v => v != null).sort((a, b) => a - b);
  if (!vs.length) return [{ min: -Infinity, color: SETAI_RAMP[0], label: "データなし" }];
  const edges = [];
  for (let i = 1; i < 6; i++) {
    const e = vs[Math.min(vs.length - 1, Math.floor(vs.length * i / 6))];
    if (!edges.length || e > edges[edges.length - 1]) edges.push(e);
  }
  const fmt = v => unit === "%" ? (Math.round(v * 10) / 10) + "%" : Math.round(v).toLocaleString();
  const bins = [];
  for (let i = edges.length; i >= 0; i--) {
    const lo = i === 0 ? -Infinity : edges[i - 1];
    const hi = i === edges.length ? null : edges[i];
    const color = SETAI_RAMP[Math.round(i * (SETAI_RAMP.length - 1) / Math.max(1, edges.length))];
    const label = i === 0 ? `〜${fmt(edges[0])}未満`
      : hi == null ? `${fmt(lo)}〜`
      : `${fmt(lo)}〜${fmt(hi)}未満`;
    bins.push({ min: lo, color, label });
  }
  return bins;
}
function setaiBinColor(bins, v){
  if (v == null) return SETAI_NODATA;
...
function addSetaiLayers(map, opts) {
  ...
  function popupHtml(p) {
    const tt = setaiTT(p);
    const gray = (t) => `<span style="font-size:10.5px;color:#888">${t}</span>`;
    const row = (label, v, unit, extra) =>
    ...
    if (p.hi) {
      /* データなしの理由は生成時に区別している(make_setai_geojson.py の HI_*)。
         「少人口のため」とは書かない: 補完的秘匿(近隣の小規模地域を守るための秘匿)では
         人口1万人超の区域も秘匿される(例: 品川区八潮五丁目) */
      const why = p.hi === 2
        ? "※同一町名として合算した集計単位の一部が国勢調査で秘匿処理されているため、男女・外国人・年齢別の内訳は表示していません"
        : p.hi === 3
        ? "※この区域に対応する統計表の行がありません(男女・外国人・年齢別はデータなし)"
        : "※この区域の男女・外国人・年齢別の値は、国勢調査の秘匿処理により近隣地域と合算して公表されており、単独の値は公表されていません";
      html += `</table>` + gray(why);
    } else {
      html += `<tr><td style="color:#555;padding-right:8px">男 / 女</td><td style="text-align:right"><b>${setaiFmt(p.m, "人")}</b> / <b>${setaiFmt(p.f, "人")}</b>${p.m != null ? "人" : ""}</td></tr>` +
        row("外国人", p.gk, "人", p.gk != null ? gray(` (${setaiFmt(setaiPct(p.gk, tt), "%")})`) : "") +
        row("日本人(概算)", p.gk == null ? null : tt - p.gk, "人") +
        row("15歳未満", p.a0, "人", p.a0 != null ? gray(` (${setaiFmt(setaiPct(p.a0, tt), "%")})`) : "") +
        row("15〜64歳", p.a1, "人") +
        row("65歳以上", p.a2, "人", p.a2 != null ? gray(` (${setaiFmt(setaiPct(p.a2, tt), "%")})`) : "") +
        row("&nbsp;うち75歳以上", p.a75, "人") + `</table>`;
      if (p.ag) {
        html += `<details style="margin-top:3px"><summary style="cursor:pointer;font-size:11px;color:#555">年齢5歳階級の内訳</summary>` +
          `<table style="border-collapse:collapse;font-size:11px">` +
          p.ag.map((v, i) => `<tr><td style="color:#555;padding-right:8px">${SETAI_AGES[i]}</td><td style="text-align:right">${v.toLocaleString()}人</td></tr>`).join("") +
          (p.nf ? `<tr><td style="color:#555;padding-right:8px">年齢不詳</td><td style="text-align:right">${p.nf.toLocaleString()}人</td></tr>` : "") +
          `</table></details>`;
      }
      const notes = [];
    ...
    if (p.units > 1) html += `<br>` + gray(`※国勢調査の集計単位${p.units}区分を合算(同一町名)`);
    return html + `<br>` + gray("2020年国勢調査時点。" + SETAI_CREDIT);
  }
  function styleOf(p) {
  ...
  ctl.onAdd = () => {
    ...
    /* 指標セレクタは stBody の外(=innerHTMLで作り直されない場所)に置く。
       検索欄と同じ理由で、開閉のたびに選択状態が飛ぶのを防ぐ */
    const metricOptions = () => {
      const grps = [];
      SETAI_METRICS.forEach(m => {
        let g = grps.find(x => x.name === m.grp);
        if (!g) { g = { name: m.grp, items: [] }; grps.push(g); }
        g.items.push(m);
      });
      return grps.map(g => `<optgroup label="${g.name}">` +
        g.items.map(m => `<option value="${m.id}"${m.id === curMetric.id ? " selected" : ""}>${m.label}</option>`).join("") +
        `</optgroup>`).join("");
    };
    panelWrap.innerHTML =
  ...
  function legendBody(body) {
    /* 表示中の指標に合わせた凡例。秘匿等データなし(グレー)の行は常に添える */
    const hasNoData = curMetric.id !== "setai" && curMetric.id !== "jinko";
    body.innerHTML = `<div style="font-weight:700;margin-bottom:2px">🏠 ${metricTitle()}(2020国勢調査)</div>` +
  ...
  function refreshLegend() {
    ...
    if (active && !legendCtl) {
      ...
      legendCtl.onAdd = () => {
        ...
        legendBox = addBoxCollapse(div, "🏠 世帯数・人口", null, legendBody);
        return div;
      };
      legendCtl.addTo(map);
      if (legendBox) legendBox.setOpen(true); /* 出るときは必ず開いた状態で */
    } else if (active && legendBox) {
      if (legendBox.isOpen()) legendBox.redraw();   /* 指標変更を凡例に反映 */
